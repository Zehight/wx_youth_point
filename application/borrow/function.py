...
from service.models import Borrow,User,Binding
...
def create_func(**kwargs):
    ...
    if binding:
        ...
        Borrow.create(book_name=kwargs['book_name'], create_by=user.id)
        return "操作成功", "数据添加成功"
    else:
...
def update_func(**kwargs):
    borrow = Borrow.get(id=kwargs['id'])
    return_time = datetime.now()
    kwargs['return_time'] = return_time
    borrow.update(**kwargs)
    return "操作成功","数据修改成功"


# 分页查询列表
def getlist_func(**kwargs):
    result = Borrow.search(**kwargs)
    return "操作成功",result
